 ...
 * fft.h
 *
 * Copyright (c) 2022 Li Zhuo
 *
 * Author: Li Zhuo
 * Email: lztony@stu.pku.edu.cn
 *
...
SDK_EXTERN void free_fft_transformer(FFTTransformer *transformer);

SDK_EXTERN void fft_forward(FFTTransformer *transformer, FFT_PRECISION *input);

SDK_EXTERN void fft_backward(FFTTransformer *transformer, FFT_PRECISION *input);

FFTCosqTransformer *create_fft_cosq_transformer(int signal_length, int scale_output);

SDK_EXTERN void free_cosq_fft_transformer(FFTCosqTransformer *transformer);

SDK_EXTERN void fft_cosq_forward(FFTCosqTransformer *transformer, FFT_PRECISION *input);

SDK_EXTERN void fft_cosq_backward(FFTCosqTransformer *transformer, FFT_PRECISION *input);
